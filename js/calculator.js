...
function calc(event){
    try{
        let armyMonth = document.querySelector("#armyMonth").innerHTML;   
    }

    //복무형태의 값이 null일 경우 alert 발생
    catch(TypeError){
        alert("복무 형태를 설정해주세요.");
        return;
    }
    ...
    if (leftPercent >= 100) { 
        ...
        result_2.innerHTML = `<p>${currentDate.getFullYear()}년 ${Number(currentDate.getMonth() + 1)}월 ${currentDate.getDate()}일을 기준으로 <b>${leftPercent}</b>% 지났습니다.</p>`;
    }
